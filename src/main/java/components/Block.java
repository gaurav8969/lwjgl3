...
public abstract class Block extends Component{
    ...
    public void update(float dt){
        if (doBopAnimation) {
            if (bopGoingUp) {
                if (this.gameObject.tf.position.y < topBopLocation.y) {
                    this.gameObject.tf.position.y += bopSpeed * dt;
                } else {
                    bopGoingUp = false;
                }
            } else {
                if (this.gameObject.tf.position.y > bopStart.y) {
                    this.gameObject.tf.position.y -= bopSpeed * dt;
                } else {
                    this.gameObject.tf.position.y = this.bopStart.y;
                    bopGoingUp = true;
